...
# ──────────────────────────────────────────
BOT_TOKEN = "8747604242:AAHxDeY0V4FDPxAsHJl4ZG6uXu0UrpE2mNw"
...
class AdminSt(StatesGroup):
    ...
    bcast_msg = State()
    set_loc = State()
    clear_confirm = State()   # Tozalashni tasdiqlash uchun

# ──────────────────────────────────────────
# 🗄 POSTGRESQL BAZA (YURAK)
# ──────────────────────────────────────────
pool = None
...
async def init_db():
    ...
    async with pool.acquire() as db:
        await db.execute("""
        CREATE TABLE IF NOT EXISTS users (id SERIAL PRIMARY KEY, tg_id BIGINT UNIQUE, name TEXT, phone TEXT, is_blocked INT DEFAULT 0, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP);
        CREATE TABLE IF NOT EXISTS admins (id SERIAL PRIMARY KEY, tg_id BIGINT UNIQUE, name TEXT, role TEXT, is_active INT DEFAULT 1);
        CREATE TABLE IF NOT EXISTS categories (id SERIAL PRIMARY KEY, name TEXT, is_active INT DEFAULT 1);
        CREATE TABLE IF NOT EXISTS products (id SERIAL PRIMARY KEY, category_id INT, name TEXT, description TEXT, price INT, stock INT DEFAULT 0, photo_id TEXT, is_active INT DEFAULT 1);
        CREATE TABLE IF NOT EXISTS cart (id SERIAL PRIMARY KEY, user_id BIGINT, product_id INT, quantity INT DEFAULT 1, UNIQUE(user_id, product_id));
        CREATE TABLE IF NOT EXISTS payment_methods (id SERIAL PRIMARY KEY, type TEXT, name TEXT, details TEXT, is_active INT DEFAULT 1);
        CREATE TABLE IF NOT EXISTS orders (
            id SERIAL PRIMARY KEY, user_id BIGINT, courier_id BIGINT DEFAULT 0, status TEXT DEFAULT 'new',
            total_amount INT DEFAULT 0, delivery_fee INT DEFAULT 0, payment_method_id TEXT, phone TEXT,
            lat REAL, lon REAL, landmark TEXT, receipt_id TEXT DEFAULT '', cancel_reason TEXT DEFAULT '',
            distance REAL DEFAULT 0.0, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
        );
        CREATE TABLE IF NOT EXISTS order_items (id SERIAL PRIMARY KEY, order_id INT, product_id INT, product_name TEXT, quantity INT, price INT);
        CREATE TABLE IF NOT EXISTS reviews (id SERIAL PRIMARY KEY, user_id BIGINT, order_id INT, product_id INT, rating INT, comment TEXT);
        CREATE TABLE IF NOT EXISTS settings (key TEXT PRIMARY KEY, value TEXT);
        """)

        for adm_id in ADMINS:
            await db.execute(
                "INSERT INTO admins(tg_id,name,role) VALUES($1,'Super Admin','admin') ON CONFLICT (tg_id) DO NOTHING",
                adm_id
            )

        pay_exists = await db.fetchrow("SELECT id FROM payment_methods LIMIT 1")
        if not pay_exists:
            await db.execute(
...
async def exe(sql, params=()):
    async with pool.acquire() as db:
        await db.execute(prep_sql(sql), *params)
...
async def qall(sql, params=()):
    async with pool.acquire() as db:
        rows = await db.fetch(prep_sql(sql), *params)
...
async def check_admin(user_id: int):
    adm = await q1("SELECT 1 FROM admins WHERE tg_id=? AND is_active=1 AND role='admin'", (user_id,))
    return bool(adm)
...
def rk(*rows):
    return ReplyKeyboardMarkup(
        keyboard=[[KeyboardButton(text=t) if isinstance(t, str) else t for t in row] for row in rows],
...
async def main_kb(tg_id: int):
    ...
    if adm and adm["role"] in ["admin", "courier"]:
...
async def build_cart_msg(user_id: int):
    ...
    for i in items:
        ...
        rows.append([
            ("➖", f"cxdec_{i['product_id']}"),
            (f"{i['name'][:12]}", "noop"),
            ("➕", f"cxinc_{i['product_id']}")
        ])
...
async def cmd_start(msg: Message, state: FSMContext):
    ...
    if u and u['is_blocked']:
        return await msg.answer("❌ Hisobingiz bloklangan.")
    if not u:
...
async def reg_name(msg: Message, state: FSMContext):
    if len(msg.text) < 2:
        return await msg.answer("Iltimos, to'g'ri ism yozing:")
...
async def reg_phone(msg: Message, state: FSMContext):
    ...
    if not str(ph).startswith("+"):
        ph = "+" + str(ph)
    d = await state.get_data()
    await exe(
        "INSERT INTO users(tg_id,name,phone) VALUES(?,?,?) ON CONFLICT (tg_id) DO NOTHING",
        (msg.from_user.id, d["name"], ph)
    )
    await state.clear()
...
async def save_name(msg: Message, state: FSMContext):
    ...
    await state.clear()
    await msg.answer(
        "✅ Ismingiz tizimga saqlandi! Endi menyudan bemalol foydalanishingiz mumkin.",
        reply_markup=await main_kb(msg.from_user.id)
    )
...
async def cmd_catalog(msg: Message, state: FSMContext):
    if await check_user(msg, state) == "blocked":
        return
    if not await is_shop_open():
        return await msg.answer("🔴 Kechirasiz, do'kon hozir yopiq. Keyinroq urinib ko'ring.")
    cats = await qall("SELECT * FROM categories WHERE is_active=1")
    if not cats:
        return await msg.answer("Bo'limlar hozircha bo'sh.")
    await msg.answer(
...
async def show_cat(call: CallbackQuery):
    ...
    if not prods:
        return await call.answer("Bo'lim bo'sh.", show_alert=True)
    kb = ik(
        *[[(f"🛒 {p['name']} - {fmt(p['price'])} so'm", f"prod_{p['id']}")] for p in prods],
        [("🔙 Orqaga", "back_cats")]
    )
    try:
...
async def show_prod(call: CallbackQuery):
    ...
    btns = [[("➖", f"cdec_{pid}"), (f"🛒 {qty} ta", "noop"), ("➕", f"cinc_{pid}")]] if qty > 0 else [[("🛒 Savatga qo'shish", f"cinc_{pid}")]]
...
async def cart_inc(call: CallbackQuery):
    ...
    ci = await q1("SELECT quantity FROM cart WHERE user_id=? AND product_id=?", (call.from_user.id, pid))
    if p and ci and ci["quantity"] >= p["stock"]:
        return await call.answer(f"Omborda faqat {p['stock']} ta bor!", show_alert=True)
    await exe(
        "INSERT INTO cart(user_id,product_id,quantity) VALUES(?,?,1) ON CONFLICT(user_id,product_id) DO UPDATE SET quantity=cart.quantity+1",
...
async def do_search(msg: Message, state: FSMContext):
    res = await qall(
        "SELECT * FROM products WHERE is_active=1 AND name ILIKE ? LIMIT 10",
        (f"%{msg.text.strip()}%",)
    )
    if not res:
        return await msg.answer("❌ Topilmadi. Boshqa so'z yozing:")
    await msg.answer(
        "🔍 Natijalar:",
...
async def cxinc(call: CallbackQuery):
    ...
    ci = await q1("SELECT quantity FROM cart WHERE user_id=? AND product_id=?", (call.from_user.id, pid))
    if p and ci and ci["quantity"] >= p["stock"]:
        return await call.answer("Omborda yetarli emas!", show_alert=True)
    await exe(
...
async def checkout_start(call: CallbackQuery, state: FSMContext):
    if not await is_shop_open():
        return await call.answer("🔴 Kechirasiz, do'kon hozir yopiq.", show_alert=True)
    ...
    for i in items:
        if i["quantity"] > i["stock"]:
            return await call.answer(f"❌ {i['name']} omborda yetarli emas!", show_alert=True)
    try:
        await call.message.delete()
    except:
        pass
    btn = [
        KeyboardButton(text="📍 Xaritadan lokatsiya yuborish", request_location=True),
        KeyboardButton(text="❌ Bekor qilish")
    ]
    await call.message.answer(
        "📍 <b>Xaritadan aniq lokatsiyangizni yuboring:</b>",
        reply_markup=rk(btn)
    )
...
async def checkout_loc(msg: Message, state: FSMContext):
    if not msg.location:
        return await msg.answer("❌ Iltimos, pastdagi '📍 Lokatsiya yuborish' tugmasini bosing!")
    await state.update_data(lat=msg.location.latitude, lon=msg.location.longitude)
    await msg.answer(
        "🏠 Endi aniq manzil va <b>Mo'ljalni (orientir)</b> yozing:\n<i>(Masalan: 4-dom, Makro orqasi)</i>",
    ...
    if pay_data == "cash":
        ...
        if pm['type'] == 'card':
            await call.message.answer(
                ...
                f"<i>(Yo'l haqi alohida hisoblanadi)</i>\n\n"
                f"Karta: <code>{pm['details']}</code>\n\n"
                f"📸 <b>To'lov chekini rasmga olib shu yerga yuboring:</b>",
                reply_markup=CANCEL_KB
            )
            await state.set_state(Checkout.receipt)
        elif pm['type'] in ['click', 'payme']:
            await call.message.answer(
                f"📱 <b>{pm['name'].upper()} Tizimi orqali to'lov</b>\n\n"
                f"🛍 Buyurtma summasi: <b>{fmt(tot)} so'm</b>\n"
                "<i>(API ga ulanish kutilyapti... Hozircha test rejimida)</i>\n\n",
                reply_markup=ik([("✅ To'lovni tasdiqlash (Test)", "force_checkout")])
            )
    await call.answer()
...
async def checkout_receipt(msg: Message, state: FSMContext):
    ...
    await finalize_order(
        msg, state, msg.from_user.id, pm['name'],
        d['tot'], d['lat'], d['lon'], d['landmark'], d['items'], msg.photo[-1].file_id
    )
...
async def force_checkout(call: CallbackQuery, state: FSMContext):
    ...
    pm = await q1("SELECT name FROM payment_methods WHERE id=?", (int(d['pm_id']),))
    await finalize_order(
        call.message, state, call.from_user.id, f"{pm['name']} (Elektron)",
        d['tot'], d['lat'], d['lon'], d['landmark'], d['items'], ""
    )
    try:
        await call.message.delete()
    except:
...
async def finalize_order(msg, state, uid, pay_name, tot, lat, lon, landmark, items, receipt_id):
    ...
    await exe(
        "INSERT INTO orders(user_id, total_amount, payment_method_id, phone, lat, lon, landmark, receipt_id, distance) "
        "VALUES(?,?,?,?,?,?,?,?,?)",
...
async def adm_direct_msg_start(call: CallbackQuery, state: FSMContext):
    target_uid = int(call.data[7:])
    await state.update_data(target_uid=target_uid)
    await call.message.answer(
        "📝 Mijozga xabaringizni yozing:\n<i>(Masalan: Lokatsiyani qaytadan tashlang)</i>",
        reply_markup=CANCEL_KB
    )
    await state.set_state(AdminSt.custom_msg)
    await call.answer()

@router.message(AdminSt.custom_msg)
async def adm_direct_msg_send(msg: Message, state: FSMContext):
    d = await state.get_data()
    ...
    if target_uid:
        try:
            await bot.send_message(
                target_uid,
                f"👨‍💻 <b>Do'kon ma'muriyatidan xabar:</b>\n\n{msg.text}"
            )
            await msg.answer("✅ Xabar mijozga muvaffaqiyatli yetkazildi!", reply_markup=admin_kb())
        except Exception:
            await msg.answer("❌ Xatolik yuz berdi. Mijoz botni bloklagan bo'lishi mumkin.")
    await state.clear()

# ──────────────────────────────────────────
# 📦 5. BUYURTMALARIM VA SHARHLAR
# ──────────────────────────────────────────
...
async def my_orders(msg: Message, state: FSMContext):
    ...
    if not ords:
        return await msg.answer("Sizda buyurtmalar yo'q.")
    for o in ords:
        ...
        if o["status"] == "new":
            ...
            if not await q1("SELECT id FROM reviews WHERE order_id=?", (o['id'],)):
                btns.append([("⭐ Baho berish", f"ureview_{o['id']}")])
        if btns:
...
async def ucancel(call: CallbackQuery):
    oid = int(call.data[8:])
    await exe(
        "UPDATE orders SET status='cancelled', cancel_reason='Mijoz ozi bekor qildi' WHERE id=?",
        (oid,)
    )
...
async def ureview(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.edit_text(
        "⭐ Sifatga baho bering (1-5):",
        reply_markup=ik([
            ("⭐ 1", "rvstar_1"), ("⭐ 2", "rvstar_2"), ("⭐ 3", "rvstar_3"),
            ("⭐ 4", "rvstar_4"), ("⭐ 5", "rvstar_5")
        ])
    )
    await call.answer()
    adms = await qall("SELECT tg_id FROM admins WHERE role='admin' AND is_active=1")
...
async def urev_star(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.edit_text(
        "Fikringizni yozib yuboring (yoki ⏭ O'tkazish):",
...
async def urev_comment(msg: Message, state: FSMContext):
    ...
    comment = msg.text
    await exe(
        "INSERT INTO reviews(user_id, order_id, product_id, rating, comment) VALUES(?,?,0,?,?)",
        (msg.from_user.id, oid, rating, comment)
    )
    await state.clear()
    ...
    for a in adms:
        try:
            await bot.send_message(
                a['tg_id'],
                f"⭐️ <b>Yangi sharh! (Buyurtma #{oid})</b>\n\nBahosi: {rating} yulduz ⭐️\n💬 Fikr: <i>{comment}</i>"
            )
        except:
            pass
...
async def urev_skip(call: CallbackQuery, state: FSMContext):
    ...
    rating = d['rev_star']
    await exe(
        "INSERT INTO reviews(user_id, order_id, product_id, rating) VALUES(?,?,0,?)",
        (call.from_user.id, oid, rating)
    )
    await state.clear()
...
async def cmd_courier(msg: Message):
    ...
    if not adm or adm['role'] not in ["admin", "courier"]:
        return
...
async def courier_my_orders(msg: Message):
    ords = await qall(
        "SELECT * FROM orders WHERE courier_id=? AND status IN ('assigned', 'on_way')",
        (msg.from_user.id,)
    )
    if not ords:
        return await msg.answer("Hozirda sizda faol buyurtmalar yo'q.")
    for o in ords:
...
async def admin_set_fee(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer(
        "🚚 <b>Yo'lkira qancha? Nima qilamiz?</b>",
        reply_markup=ik(
...
async def admin_auto_conf(call: CallbackQuery):
    ...
    try:
        await bot.send_message(
            o['user_id'],
            f"✅ Tasdiqlandi! Yo'lkira: <b>{fmt(fee)} so'm</b>.\nKuryer qidirilmoqda 🔎"
        )
    except:
        pass
    ...
    txt = (
        f"🔥 <b>YANGI BUYURTMA #{oid}</b>\n"
        f"📍 Mo'ljal: {o['landmark']}\n"
        f"📏 Masofa: {o['distance']:.1f} km\n"
        f"💰 Yo'lkira: <b>{fmt(fee)} so'm</b>\n"
        f"Kim birinchi olsa o'shanga yoziladi!"
    )
    for c in cs:
        try:
...
async def admin_self_deliver(call: CallbackQuery):
    ...
    fee = int(parts[2])
    await exe(
        "UPDATE orders SET delivery_fee=?, status='on_way', courier_id=? WHERE id=?",
        (fee, call.from_user.id, oid)
    )
...
async def admin_cancel_order(call: CallbackQuery):
    oid = int(call.data[8:])
    await exe(
        "UPDATE orders SET status='cancelled', cancel_reason='Admin bekor qildi' WHERE id=?",
        (oid,)
    )
    await return_stock(oid)
    ...
    o = await q1("SELECT user_id FROM orders WHERE id=?", (oid,))
    try:
        await bot.send_message(o['user_id'], f"❌ Buyurtmangiz (#{oid}) bekor qilindi.")
    except:
        pass
    await call.answer()
...
async def courier_grab(call: CallbackQuery):
    if not await q1("SELECT 1 FROM admins WHERE tg_id=? AND role='courier'", (call.from_user.id,)):
        return
    oid = int(call.data[6:])
    o = await q1("SELECT status FROM orders WHERE id=?", (oid,))
    if o['status'] != 'pending_courier':
        return await call.answer("❌ Kechikdingiz, boshqa kuryer olib bo'ldi!", show_alert=True)
    ...
    try:
        ...
        pass
    await call.message.answer(
        f"✅ <b>Buyurtma #{oid} sizga yozildi!</b>",
...
async def courier_pickup(call: CallbackQuery):
    ...
    o = await q1("SELECT user_id FROM orders WHERE id=?", (oid,))
    try:
        await bot.send_message(o['user_id'], "🚚 <b>Kuryer yo'lga chiqdi!</b>")
    except:
        pass
    await call.message.edit_text(
        f"🚚 <b>#{oid} yo'lda!</b>",
...
async def courier_deliver(call: CallbackQuery):
    ...
    try:
        await bot.send_message(
            o['user_id'],
            "🎉 <b>Hurmatli mijoz!</b>\nBuyurtma yetkazildi. Qo'lingizga oldingizmi?",
...
# ──────────────────────────────────────────
# ⚙️ 7. ADMIN PANEL VA SOZLAMALAR
# ──────────────────────────────────────────
@router.message(F.text == "⚙️ Admin Panel")
async def cmd_admin(msg: Message):
    if not await q1("SELECT 1 FROM admins WHERE tg_id=? AND role='admin'", (msg.from_user.id,)):
        return
...
async def toggle_shop(call: CallbackQuery):
    ...
    await call.answer("🟢 Ochildi" if new_val == "1" else "🔴 Yopildi", show_alert=True)
    try:
        await call.message.delete()
    except:
        pass
...
async def set_shop_loc_start(call: CallbackQuery, state: FSMContext):
    btn = [
        KeyboardButton(text="📍 Xaritadan lokatsiya tashlash", request_location=True),
        KeyboardButton(text="❌ Bekor qilish")
    ]
    await call.message.answer(
        "Pastdagi tugma orqali do'konning yangi lokatsiyasini tashlang:",
        reply_markup=rk(btn)
    )
...
async def set_shop_loc_save(msg: Message, state: FSMContext):
    if not msg.location:
        return await msg.answer("Iltimos, lokatsiya tashlang!")
...
async def admin_staff(msg: Message):
    ...
    await msg.answer(
        "👥 <b>Xodimlar (O'chirish uchun ustiga bosing):</b>",
        reply_markup=ik(*btns)
    )
...
async def add_staff(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer("🆔 Yangi xodimning Telegram ID sini kiriting:", reply_markup=CANCEL_KB)
...
async def save_staff(msg: Message, state: FSMContext):
    ...
    await exe(
        "INSERT INTO admins(tg_id,name,role) VALUES(?,?,?) ON CONFLICT (tg_id) DO NOTHING",
        (tg_id, f"{role_uz} (Ism kutilmoqda...)", role)
    )
    await state.clear()
    ...
    try:
        await bot.send_message(
            tg_id,
            f"🎉 <b>Tabriklaymiz!</b> Siz do'konga <b>{role_uz}</b> etib tayinlandingiz!\n\n"
            f"👇 Iltimos, ma'muriyat sizni tanib olishi uchun pastdagi tugmani bosib o'z ismingizni kiriting:",
            reply_markup=ik([("✍️ Ismni kiritish", "force_name_set")])
        )
    except Exception:
        pass
...
async def admin_users_list(msg: Message):
    ...
    if not users:
        return await msg.answer("Mijozlar hozircha yo'q.")
...
async def admin_ban_menu(msg: Message, state: FSMContext):
    if not await q1("SELECT 1 FROM admins WHERE tg_id=? AND role='admin'", (msg.from_user.id,)):
        return
    await msg.answer(
        "🚫 Bloklash yoki ochish uchun Mijozning Telegram ID sini yozing:\n"
        "<i>(ID ni '👥 Mijozlar' bo'limidan nusxalashingiz mumkin)</i>",
        reply_markup=CANCEL_KB
    )
...
async def add_pay_type(call: CallbackQuery, state: FSMContext):
    await call.message.answer(
        ...
        reply_markup=ik(
            [("💳 Karta", "ptype_card")],
            [("📱 Click", "ptype_click"), ("📱 Payme", "ptype_payme")]
        )
    )
    await call.answer()
...
async def add_pay_name(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer(
        "✏️ Nomi qanday ko'rinsin? (Masalan: Humo - Toshmat):",
        reply_markup=CANCEL_KB
    )
...
async def save_pay(msg: Message, state: FSMContext):
    d = await state.get_data()
    await exe(
        "INSERT INTO payment_methods(type, name, details) VALUES(?,?,?)",
...
async def adm_cats(msg: Message):
    ...
    await msg.answer("Bo'limlar:", reply_markup=ik(*btns))
...
async def admin_cat_detail(call: CallbackQuery):
    ...
    await call.message.edit_text(
        f"📦 <b>{c['name']}</b>",
        reply_markup=ik(
            [("✏️ Nomni o'zgartirish", f"acatedit_{cid}")],
...
async def admin_cat_edit(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer("✏️ Yangi nomini kiriting:", reply_markup=CANCEL_KB)
...
async def admin_cat_edit_name_save(msg: Message, state: FSMContext):
    ...
    await state.clear()
    await msg.answer("✅ O'zgartirildi!", reply_markup=admin_kb())
...
async def adm_prods(msg: Message):
    ...
    btns = [[(f"{'✅' if p['is_active'] else '❌'} {p['name']} ({p['cname']})", f"admp_{p['id']}")] for p in prods]
...
async def add_prod_cb(call: CallbackQuery, state: FSMContext):
    cats = await qall("SELECT * FROM categories WHERE is_active=1")
    if not cats:
        return await call.answer("Bo'lim yo'q!", show_alert=True)
    await call.message.answer(
...
async def pedit_menu(call: CallbackQuery):
    pid = int(call.data[6:])
    await call.message.edit_text(
        "Nimani o'zgartiramiz?",
        reply_markup=ik(
            [("✏️ Nomi", f"pedit_name_{pid}"), ("📄 Tavsifi", f"pedit_desc_{pid}")],
            [("💰 Narxi", f"pedit_price_{pid}"), ("📦 Ombor", f"pedit_stock_{pid}")],
            [("🖼 Rasm", f"pedit_photo_{pid}"), ("🔙 Orqaga", f"admp_{pid}")]
        )
    )
    await call.answer()

@router.callback_query(F.data.startswith("pedit_"))
async def pedit_field(call: CallbackQuery, state: FSMContext):
    parts = call.data.split("_")
    field = parts[1]
    pid = int(parts[2])
    ...
    prompts = {
        ...
        "stock": "📦 Ombordagi yangi qoldiqni kiriting (Faqat raqam):",
        "photo": "🖼 Yangi rasmni yuboring:"
    }
    await call.message.answer(prompts[field], reply_markup=CANCEL_KB)
    await state.set_state(AdminSt.prod_edit_val)
    await call.answer()

@router.message(AdminSt.prod_edit_val)
async def save_pedit(msg: Message, state: FSMContext):
    d = await state.get_data()
    pid = d.get("edit_pid")
    field = d.get("edit_field")
    if field in ["price", "stock"]:
        if not msg.text or not msg.text.isdigit():
            return await msg.answer("⚠️ Iltimos, faqat raqam kiriting!\n(Yoki ❌ Bekor qilishni bosing)")
        val = int(msg.text)
    elif field == "photo":
        if not msg.photo:
    ...
    db_fields = {"name": "name", "desc": "description", "price": "price", "stock": "stock", "photo": "photo_id"}
    col = db_fields[field]
    await exe(f"UPDATE products SET {col}=? WHERE id=?", (val, pid))
    await state.clear()
    await msg.answer("✅ Mahsulot muvaffaqiyatli tahrirlandi!", reply_markup=admin_kb())
...
async def adm_stats(msg: Message):
    ...
    prod_c = (await q1("SELECT COUNT(*) AS n FROM products WHERE is_active=1"))['n']

    # ── Buyurtma holatlari ───────────────────────────────────────────
    o_new    = (await q1("SELECT COUNT(*) AS n FROM orders WHERE status='new'"))['n']
    o_way    = (await q1("SELECT COUNT(*) AS n FROM orders WHERE status='on_way'"))['n']
    o_cancel = (await q1("SELECT COUNT(*) AS n FROM orders WHERE status='cancelled'"))['n']

    # ── Bugungi yetkazilganlar ───────────────────────────────────────
    today = await q1("""
        SELECT COUNT(*) AS n, COALESCE(SUM(total_amount), 0) AS s
        FROM orders
        WHERE status = 'delivered'
          AND DATE(created_at) = CURRENT_DATE
    """)

    # ── Shu oydagi yetkazilganlar ────────────────────────────────────
    month = await q1("""
        SELECT COUNT(*) AS n, COALESCE(SUM(total_amount), 0) AS s
        FROM orders
        WHERE status = 'delivered'
    ...
    medals = ["🥇", "🥈", "🥉", "4️⃣", "5️⃣"]
    if top5:
        top5_txt = "\n\n🏆 <b>Eng ko'p sotilgan 5 ta mahsulot:</b>\n"
        for idx, row in enumerate(top5):
            top5_txt += (
                f"{medals[idx]} {row['product_name']} — "
                f"<b>{row['total_qty']} ta</b> "
                f"(<i>{fmt(row['total_sum'])} so'm</i>)\n"
            )
    else:
        top5_txt = "\n\n🏆 <b>Top mahsulotlar:</b>\n<i>Hali ma'lumot yo'q.</i>"

    text = (
        f"📊 <b>Do'kon statistikasi</b>\n\n"
        f"👥 Ro'yxatdagi mijozlar: <b>{u_c} ta</b>\n"
        f"📦 Faol mahsulotlar: <b>{prod_c} ta</b>\n\n"
        f"━━━━━━━━━━━━━━━━━━\n"
        f"📅 <b>Bugun</b> (yetkazilgan):\n"
        f"  🔢 Buyurtmalar: <b>{today['n']} ta</b>\n"
        f"  💰 Daromad: <b>{fmt(today['s'])} so'm</b>\n\n"
        f"🗓 <b>Shu oy</b> (yetkazilgan):\n"
        f"  🔢 Buyurtmalar: <b>{month['n']} ta</b>\n"
        f"  💰 Daromad: <b>{fmt(month['s'])} so'm</b>\n\n"
        f"📆 <b>Shu yil</b> (yetkazilgan):\n"
        f"  🔢 Buyurtmalar: <b>{year['n']} ta</b>\n"
        f"  💰 Daromad: <b>{fmt(year['s'])} so'm</b>\n\n"
        f"━━━━━━━━━━━━━━━━━━\n"
        f"🔄 <b>Hozirgi holat:</b>\n"
        f"  ⏳ Yangi / tasdiqlanmagan: <b>{o_new}</b>\n"
        f"  🚚 Yo'lda: <b>{o_way}</b>\n"
        f"  ❌ Bekor qilingan: <b>{o_cancel}</b>"
        f"{top5_txt}"
    )
...
async def admin_clear_menu(msg: Message):
    if not await q1("SELECT 1 FROM admins WHERE tg_id=? AND role='admin'", (msg.from_user.id,)):
        return
    await msg.answer(
        "🧹 <b>Qaysi ma'lumotlarni tozalamoqchisiz?</b>\n\n"
        "⚠️ <i>Diqqat: bu amal qaytarib bo'lmaydi!</i>",
        reply_markup=ik(
            [("📂 Bo'limlarni tozalash", "clr_categories")],
            [("📦 Mahsulotlarni tozalash", "clr_products")],
            [("🚚 Kuryerlarni tozalash", "clr_couriers")],
            [("👥 Mijozlarni tozalash", "clr_users")],
            [("📋 Buyurtmalarni tozalash", "clr_orders")],
            [("🗑 Hammasini tozalash", "clr_all")],
            [("❌ Bekor qilish", "clr_cancel")]
        )
    )

@router.callback_query(F.data == "clr_cancel")
async def clr_cancel(call: CallbackQuery):
    await call.message.edit_text("🚫 Tozalash bekor qilindi.")
    await call.answer()

@router.callback_query(F.data.startswith("clr_"))
async def clear_confirm_ask(call: CallbackQuery, state: FSMContext):
    target = call.data[4:]
    if target == "cancel":
        return
    ...
    await state.update_data(clear_target=target)
    await call.message.edit_text(
        f"⚠️ <b>Haqiqatan ham <u>{label}</u> ni tozalashni xohlaysizmi?</b>\n\n"
        f"Bu amal <b>qaytarib bo'lmaydi!</b>\n"
        f"Tasdiqlash uchun quyidagi tugmani bosing:",
        reply_markup=ik(
            [("✅ Ha, tozalayman!", f"clrdo_{target}")],
            [("❌ Yo'q, bekor", "clr_cancel")]
        )
    )
    await call.answer()

@router.callback_query(F.data.startswith("clrdo_"))
async def clear_execute(call: CallbackQuery, state: FSMContext):
    if not await q1("SELECT 1 FROM admins WHERE tg_id=? AND role='admin'", (call.from_user.id,)):
    ...
    async with pool.acquire() as db:
        try:
            if target == "categories":
                ...
                result_txt = "📂 Bo'limlar tozalandi."

            elif target == "products":
                await db.execute("TRUNCATE TABLE products RESTART IDENTITY CASCADE")
                result_txt = "📦 Mahsulotlar tozalandi."

            elif target == "couriers":
                # Faqat kuryer rolindagi xodimlarni o'chiramiz, adminlarni saqlaymiz
                await db.execute("DELETE FROM admins WHERE role = 'courier'")
                result_txt = "🚚 Kuryerlar ro'yxati tozalandi."

            elif target == "users":
                await db.execute("TRUNCATE TABLE users RESTART IDENTITY CASCADE")
                result_txt = "👥 Mijozlar tozalandi."

            elif target == "orders":
                await db.execute("TRUNCATE TABLE orders RESTART IDENTITY CASCADE")
                await db.execute("TRUNCATE TABLE order_items RESTART IDENTITY CASCADE")
                await db.execute("TRUNCATE TABLE reviews RESTART IDENTITY CASCADE")
                result_txt = "📋 Buyurtmalar, buyurtma elementlari va sharhlar tozalandi."

            elif target == "all":
                # Tartib muhim: bog'liq jadvallarni avval tozalaymiz
                await db.execute("TRUNCATE TABLE reviews RESTART IDENTITY CASCADE")
                await db.execute("TRUNCATE TABLE order_items RESTART IDENTITY CASCADE")
                await db.execute("TRUNCATE TABLE orders RESTART IDENTITY CASCADE")
                await db.execute("TRUNCATE TABLE cart RESTART IDENTITY CASCADE")
                await db.execute("TRUNCATE TABLE products RESTART IDENTITY CASCADE")
                await db.execute("TRUNCATE TABLE categories RESTART IDENTITY CASCADE")
                await db.execute("TRUNCATE TABLE users RESTART IDENTITY CASCADE")
                # Kuryerlarni o'chiramiz, asosiy adminlar qoladi
                await db.execute("DELETE FROM admins WHERE role = 'courier'")
                result_txt = "🗑 Barcha ma'lumotlar (kuryerlar, mijozlar, mahsulotlar, buyurtmalar) tozalandi.\n✅ Asosiy adminlar saqlab qolindi."

            else:
                result_txt = "❓ Noma'lum buyruq."

            await call.message.edit_text(f"✅ <b>Muvaffaqiyatli!</b>\n\n{result_txt}")

        except Exception as e:
            await call.message.edit_text(f"❌ <b>Xatolik yuz berdi:</b>\n<code>{e}</code>")
...
async def send_bcast(msg: Message, state: FSMContext):
    ...
    for u in users:
        try:
            await bot.send_message(u['tg_id'], f"📢 {msg.text}")
            ok += 1
        except:
            pass
...
async def main():
    ...
    print("🚀 Enterprise Pro Bot PostgreSQL & Redis bilan ishga tushdi...")
